...
def combination_selection(doc_sent_list, abstract_sent_list, summary_size):
    def _rouge_clean(s):
        return re.sub(r'[^a-zA-Z0-9äöüÄÖÜß ]', '', s)
...
class BertData():
    def __init__(self, args):
        self.args = args
        self.tokenizer = BertTokenizer.from_pretrained("dbmdz/bert-base-german-cased", do_lower_case=True)
...
def format_to_lines(args):
    ...
    for f in glob.glob(pjoin(args.raw_path, '*.json')):
        ...
        name_splits = real_name.split('\\')
        if len(name_splits) > 1:
            real_name = name_splits[1]
        # print(real_name)
