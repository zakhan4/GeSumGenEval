...
# path to store some temporary files
temp_path = './temp'
...
def get_candidates(tokenizer, cls, sep_id, idx):
    ...
    truncate_length = 5
    if len(data['text']) < truncate_length:
        truncate_length = len(data['text'])
    sent_id = sent_ids[idx]['sent_id'][:truncate_length]
    indices = list(combinations(sent_id, 1))
    ...
    for i in indices:
        ...
        for j in i:
            try:
                ...
                print("Exception occured at index: " + str(idx))
                return
        text = "\n".join(dec)
        summary = "\n".join(data['summary'])
        rouge_score = rouge.get_scores(text, summary)
        score_list = [v['f'] for k, v in rouge_score[0].items()]
        score.append((i, sum(score_list) / len(score_list)))
    ...
    for summary in candidate_summary:
        token_ids = tokenizer.encode(summary, add_special_tokens=False, truncation=True, max_length=MAX_LEN)
        token_ids += sep_id
    ...
    text = text[:MAX_LEN]
    text = ' '.join(text)
    token_ids = tokenizer.encode(text, add_special_tokens=False, truncation=True, max_length=MAX_LEN)
    token_ids += sep_id
...
def get_candidates_mp(args):
    ...
    with mp.Pool() as pool:
        for _ in tqdm.tqdm(pool.imap_unordered(get_candidates(tokenizer, cls, sep_id), range(n_files), chunksize=64)):
            pass
