...
def train_model(args):
    ...
    valid_set = datasets.datasets['valid']
    ...
    val_metric = [ValidMetric(save_path=args.save_path, data=read_jsonl(data_paths['valid']))]
...
def test_model(args):
    ...
    print('Current model is {}'.format(args.test_from))

    # load model
    model = torch.load(args.test_from)

    # configure testing
    test_metric = MatchRougeMetric(data=read_jsonl(data_paths['test']), save_path=args.save_path, n_total = len(test_set))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_from',
                        help='path of the model checkpoint to train from', type=str)
    parser.add_argument('--test_from',
                        help='path of the model checkpoint to test from', type=str)
    ...
    parser.add_argument('--candidate_num', default=25,
